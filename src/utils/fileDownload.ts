...
export const downloadMarkdown = (content: string, filename: string) => {
  ...
  downloadTextFile(content, fullFilename, 'text/markdown');
};

/**
 * テンプレートファイル名を生成する
 */
export const generateTemplateFilename = (templateName: string, deliverableTitle: string): string => {
  const sanitizedTemplateName = templateName.replace(/[^\w\s-]/g, '').trim();
  const sanitizedDeliverableTitle = deliverableTitle.replace(/[^\w\s-]/g, '').trim();
 ...
 */
export const downloadDeliverablePackage = async (deliverables: Array<{
  filename: string;
  content: string;
